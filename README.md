...
### Automatic mode
This is the active state (1) for the actuator. The thermostat function is in effect and compensation based on outdoor temperature is performed. The actuator is operated completely based on parameter settings. The actuator is run with a fixed 4 s pulse interval.
...
### Semi-automatic mode
This is the active state (2) for the actuator. The thermostat function is in effect but no compensation based on outdoor temperature is performed. The actuator is operated with a static set-point value through the front-end. The actuator is run with a fixed 4 s pulse interval.
...
|**Temperature compensation set-points**|Generally known as “heating curve”. A set of temperature points between which the target supply line temperature is calculated based on meassured outdoor temperature. Calculation principle is linear interpolation. The target supply line temperature is constant for outdoor temperatures above the highest set-point. Available points are p1 to p6.|[Table](#default-compensation-set-points)|
...
![Prototype PCB](/images/esp32_ssr_mvc_pcb.png)
<br>
*Prototype printed circuit board for a 3 unit 53.4 mm DIN enclosure.*
